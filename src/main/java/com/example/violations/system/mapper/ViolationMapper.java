...
public class ViolationMapper {
    ...
    public ViolationResponseDto toDto(Violation violation) {
        ...
        if (violation.getCarImage() != null && !violation.getCarImage().isEmpty()) {
            List<String> carImagePaths = new ArrayList<>();
            for (CarImage carImage : violation.getCarImage()) {
                carImagePaths.add(carImage.getImagePath());
            }
            dto.setCarImages(carImagePaths);
        }

        if (violation.getInspector() != null) {
